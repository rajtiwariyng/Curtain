        ...
        if(!empty($appointment_id)){
            ...
            if(!empty($appointment)){
                ...
                if ($quotation) {
                    ...
                    $order_data = Order::select('id')->where([
                    ...
                    if ($quotation_data) {
                        foreach ($quotation_data as $datas) {
                            if ($datas['price'] != '' && $datas['gst_percentage'] != '') {
                                $total += (
                                    ($datas['price'] * $datas['qty'])  // Base price times quantity
                                    + ($datas['price'] * $datas['qty'] * $datas['gst_percentage'] / 100)  // GST calculation based on price, qty, and GST percentage
                                    - ($datas['price'] * $datas['qty'] * $datas['discount'] / 100)  // Discount based on price, qty, and discount percentage
                                );
                            }
                        }
                    }
                    ...
                    $quotation_items['total_order_value'] =  $total ?? 0;					
                    // $quotation_items['total_order_value'] =  20;					

                    $quotation_items['franchise_id'] = $appointment->franchise_id;

                    $quotation_items['appointment_id'] = $appointment->id;
                    $quotation_items['order_id'] = $order_data['id'];
