...
public function store(Request $request)
{
 ...
 {  
    $products = Product::with('ProductType', 'Supplier', 'SupplierCollection', 'SupplierCollectionDesign');
    ...
    foreach ($filters as $key => $column) {
        $value = request()->get($key);  // Get the value of the query parameter
        
        if ($value && $value !== 'Select') {
            // Apply the 'whereIn' condition for arrays, and 'where' for single values
            if (is_array($value)) {
    ...
    $productTypes=ProductType::all();
     ...
     return view('admin.product.index', compact('products','usages','colours','compositions','designTypes','types','suppliers','supplierCollection','supplierCollectionDesign','productTypes'));
 }
 ...
 function data_log()
 {
        ...
        $retailers_query = User::select('id', 'retailer_name', 'uniquecode', 'gst', 'pan', 'email', 'mobile', 'image', 'status', 'created_at')
        ...
        if (!empty($_POST['filter_status'])) {
            $retailers_query->where('status', 'like', '%'.$_POST['filter_status'].'%');
        }

        if (!empty(request('start_date'))) {
            $retailers_query->where('created_at', '>=', date('Y-m-d', strtotime(request('start_date'))));
        }
        if (!empty(request('end_date'))) {
            $retailers_query->where('updated_at', '<=', date('Y-m-d', strtotime(request('end_date'))));
        }
        ...
        if (!empty($post['search']['value'])) {
            ...
            $retailers_query->where(function ($query) use ($searchValue) {
                $query->where('retailer_name', 'LIKE', "%$searchValue%")
        ...
        $retailers = $retailers_query->offset($start)->limit($limit)->get();
        ...
        foreach ($retailers as $key => $retailer) {
            ...
            $data_array[] = [
                $key + 1,
                ($retailer['image'] != 'https://redi-bucket.s3.ap-south-1.amazonaws.com/redi/users/image/Distri_109_2024-08-19%2010%3A00%3A26.png' && !empty($retailer['image'])) ?
                '<a href="' . $retailer['image'] . '" class="fancybox"><img src="' . $retailer['image'] . '" height="30px"></a> ' . ucwords($retailer['retailer_name']) . ' ( ' . $retailer['uniquecode'] . ' )'
                :
                '<img src="" >' . ucwords($retailer['retailer_name']) . ' ( ' . $retailer['uniquecode'] . ' )',
                $retailer['gst'],
                $retailer['pan'],
                $retailer['email'],
                $retailer['mobile'],
                isset($retailer['credit_data']['credit_limit']) ? $retailer['credit_data']['credit_limit'] : 0,

                '<div class="toggle-btn ' . ($retailer['status'] == 'Approved' ? 'active' : '') . '">
                    <input type="checkbox" ' . ($retailer['status'] == 'Approved' ? 'checked' : '') . ' class="cb-value" data-id="' . custom_encode($retailer['id']) . '" />
                    <span class="round-btn"></span></div>',
                $retailer['time'],
                '<div class="assign_edit"><a href="rm/change_mapping/' . custom_encode($retailer['id']) . '" class="fancybox ajax"><button class="redi_btn xs"><i class="fa fa-edit"></i>Assign</button></a> 
                <a href="' . ('retailer/edit/') . custom_encode($retailer['id']) . '" class=""><button class="redi_btn xs"><i class="fa fa-edit"></i> Edit</button></a></div>',
                '<a href="field-force/rm/edit_rm_history/' . custom_encode($retailer['id']) . '" class="fancybox ajax"><button class="btn btn-info btn-sm"><i class="fa fa-history"></i></button></a>'
